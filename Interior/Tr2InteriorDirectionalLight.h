...
#pragma once
#ifndef Tr2InteriorDirectionalLight_H
#define Tr2InteriorDirectionalLight_H
...
// --------------------------------------------------------------------------------------
// Description:
//   Tr2InteriorDirectionalLight represents a directional light that affects the entire 
//   scene. It uses several shadow LODs for good shadows. Lights are managed at the 
...
// --------------------------------------------------------------------------------------
class Tr2InteriorDirectionalLight :
	public INotify,
	public IInitialize,
...
public:
	// Constructor
	Tr2InteriorDirectionalLight( IRoot* lockobj = 0 );
	// Destructor
	~Tr2InteriorDirectionalLight();
	...
	void GetLocalBoundingBox( Vector3& minBounds, Vector3& maxBounds ) const;
	...
	unsigned int GetFramesSinceShadowUpdate( unsigned int shadowMapIndex ) const { return m_framesSinceShadowUpdate[shadowMapIndex]; }
	// Sets the number of frames since the shadow was updated for spot lights
	void SetFramesSinceShadowUpdate( unsigned int shadowMapIndex, unsigned int framesSinceShadowUpdate ) { m_framesSinceShadowUpdate[shadowMapIndex] = framesSinceShadowUpdate; }
...
private:
	...
	float GetShadowLODSize( unsigned shadowIndex ) const;
	...
	// Light direction
	Vector3 m_direction;
	// Light space to world transform
	Matrix m_transform;
	// World to light space transform
	Matrix m_transformInv;
	// Bounds of the scene in local space
	Vector3 m_minBounds;
	// Bounds of the scene in local space
	Vector3 m_maxBounds;
	...
	// Use explicitely set bounds or automatically generated from all scene cells
	bool m_useExplicitBounds;
	// Min explicit bounds
	Vector3 m_explicitBoundsMin;
	// Max explicit bounds
	Vector3 m_explicitBoundsMax;
	...
	std::map<Tr2InteriorCell*, std::pair<Vector3, Vector3>> m_cellBounds;
	...
	Matrix m_projectionMatrix[6];
	// Shadow LOD rectangles inside view frustum (used for rendering)
	Vector4 m_shadowRect[6];
	// New shadow LOD rectangles inside view frustum requested for shadow updates
	Vector4 m_newShadowRect[6];
	...
	// Number of shadow LODs
	unsigned int m_shadowLODs;
	// Shadow texture for spot lights
	Tr2AtlasTexturePtr m_shadowTextures[6];
	// Size of blending region on borders of shadow LODS (0 to 1)
	float m_LODBlendRegion;
	// Show shadow LODs with different colors
	bool m_debugShadowLODs;
	// LOD distribution coeficient
	float m_LODDistribution;
...
TYPEDEF_BLUECLASS( Tr2InteriorDirectionalLight );
BLUE_DECLARE_VECTOR( Tr2InteriorDirectionalLight );


#endif // Tr2InteriorDirectionalLight_H