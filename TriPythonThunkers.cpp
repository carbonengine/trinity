...
PyObject* ITriRenderObject_Thunk::PyReload(PyObject* args)
{	
	ReloadResources();
...
PyObject* ITriRenderObject_Thunk::PyUpdate(PyObject* args)
{	
	PyObject* t;
	if (!PyArg_ParseTuple(args, "O", &t))
		return NULL;
	...
	if (PyLong_Check(t))
	{		
		Update(Be::Time(PyLong_AsLongLong(t)));
	}	
	else
	{
		BeOS->SetError(BEDEF, Clsid(), 
			"arg must be of type LongLong (Be::Time)");
		return nullptr;
	}
...
PyObject* ITriRenderObject_Thunk::PyRender(PyObject* args)
{	
	int p1, p2;
...
PyObject* ITriRenderObject_Thunk::PyRebuildCachedData(
	PyObject* args
	)
{
	RebuildCachedData();
