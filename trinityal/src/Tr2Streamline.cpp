...
bool IsAvailable( StreamlineUtils::StreamlinePlugin plugin, unsigned adapterIndex )
{
	...
	Tr2VideoAdapterInfo::GetAdapterInfo(adapterIndex, videoAdapterInfo);
	...
	if( result != sl::Result::eOk )
	{
		...
		case sl::Result::eErrorMissingOrInvalidAPI:
			CCP_LOGWARN("Graphics api not supported for %s", info.pluginName);
			break;
		default:
...
void Toggle( StreamlineUtils::StreamlinePlugin plugin, unsigned adapterIndex, bool enable )
{
	...
	if( !IsAvailable( plugin, adapterIndex ) || !IsValid( plugin, info ) )
	{
		return;
	}
	...
	if( plugin == StreamlineUtils::SP_DLSSG )
	{
		...
		PrepareReflex( enable, adapterIndex );
	}
	...
	case StreamlineUtils::SP_DLSSG:
		if( !s_dlssPlugin )
		{
			Toggle( StreamlineUtils::StreamlinePlugin::SP_DLSS, adapterIndex, true );
		}
...
void PrepareReflex( bool enable, unsigned adapterIndex )
{
	if( enable )
	{
		Toggle( StreamlineUtils::StreamlinePlugin::SP_REFLEX, adapterIndex, true );
	}
	...
	if( !enable )
	{
		Toggle( StreamlineUtils::StreamlinePlugin::SP_REFLEX, adapterIndex, false );
	}
