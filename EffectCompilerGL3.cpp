...
//   false Otherwise
// --------------------------------------------------------------------------------------
static bool AsmToGLES3( const char* source, std::string& glCode, const StageInput& stage )
{
	using std::endl;
...
#define EXPECT_TOKEN( stream, token, expectedType )  {			\
	...
	if( stage.type == PIXEL_STAGE )
	{
		outputRemap[MakeInlineString( "oC0" )] = MakeInlineString( "FragData[0]" );
		outputRemap[MakeInlineString( "oC1" )] = MakeInlineString( "FragData[1]" );
		outputRemap[MakeInlineString( "oC2" )] = MakeInlineString( "FragData[2]" );
		outputRemap[MakeInlineString( "oC3" )] = MakeInlineString( "FragData[3]" );
	...
	for( size_t tokenIndex = 0; tokenIndex < tokens.size(); )
	{
		...
		else if( command.name == MakeInlineString( "dcl" ) )
		{
			...
			else if( reg.name.start[0] == 'o' && stage.type == VERTEX_STAGE )
			{
				...
				{
					declOs << "out vec4 " << command.modifier << ';' << endl;
				}
			}
			else if( stage.type == VERTEX_STAGE )
			{
				...
				name.start++;
				declOs << "in vec4 attr" << name << ';' << endl;
				os << reg.name << "=attr" << name << ';' << endl;
			}
			else // PIXEL_STAGE input
			{
				declOs << "in vec4 " << command.modifier << ';' << endl;
	...
	else
	{
		...
		os << "\n#ifdef PS\n";
		// Alpha test
		os << "float av=floor(clamp(FragData[0].a,0.0,1.0)*255.0+0.5);" << endl
			<< "if(ssi.z==0.0)" << endl 
			<< "{" << endl 
