	...
	case NT_NAME_DECLARATION:
		{
			...
			for( auto it = node->GetSymbol()->registerSpecifier.begin(); it != node->GetSymbol()->registerSpecifier.end(); ++it )
			{
				...
				if( reg.registerType == 'b' || reg.registerType == 'B' ||
			...
			if( node->GetChildOrNull( 1 ) && node->GetChild( 1 )->GetNodeType() != NT_SAMPLER_STATE_LIST )
			{
				os << " = ";
				PrintHLSL11( os, node->GetChild( 1 ), level );
			}
...
static PatchAction PatchShader( InputStageType type, bool patchOutput, ASTNode* callNode, ParserState& state, CodeStream& os, std::string& entryPointName, bool& hasShadowState )
{
	...
	if( patchOutput )
	{
		if( type == PIXEL_STAGE )
		{
			const char* target[] = { "color", "color0", "sv_target", "sv_target0", "s_target_output", nullptr };
		...
		{
			const char* position[] = { "position", "sv_position", "s_position", nullptr };
	...
	{
		hasShadowState = true;
		os << "cbuffer DX11ShadowStateCB : register( b10 )\n{\n";
		os << "struct {\n";
		os << "int invertedTest;\n";
		os << "int alphaTestRef;\n";
	...
	for( size_t i = 0; i < functionHeader->GetChildrenCount(); ++i )
	{
		...
		else
		{
			...
			{
				os << "float4 " << functionHeader->GetChild( i )->GetSymbol()->name << ": S_POSITION";
			}
			else
			{
...
static bool RunProcess( const char* commandLine )
{
	...
	if( exitCode || g_printWarnings )
	{
		g_messages.AddMessage( "%s", output.c_str() );
	}
...
bool CompileOrbisShader( const char* code, const char* entryPoint, const char* profile, ID3DBlob** orbisEffectData )
{
	static const char* orbisPrefix = "#define cbuffer ConstantBuffer\n#define packed packed0\n"
	...
	{
		g_messages.AddMessage( "\\memory(0): error O0101: could not create temporary files for external Orbis compiler" );
		return false;
	}
	if( GetTempFileName( tempDir, "oi", 0, inFile ) == 0 )
	{
		g_messages.AddMessage( "\\memory(0): error O0101: could not create temporary files for external Orbis compiler" );
		return false;
	}
	if( GetTempFileName( tempDir, "oo", 0, outFile ) == 0 )
	{
		g_messages.AddMessage( "\\memory(0): error O0101: could not create temporary files for external Orbis compiler" );
		return false;
	}
	...
	FILE* input;
	if( fopen_s( &input, inFile, "wt" ) != 0 )
	{
		g_messages.AddMessage( "\\memory(0): error O0101: could not create temporary files for external Orbis compiler" );
		return false;
	}
	fputs( orbisPrefix, input );
	fputs( code, input );
	fclose( input );

	std::string orbisProfile;
	if( strncmp( profile, "vs_", 3 ) == 0 )
	{
		orbisProfile = "sce_vs_vs_orbis";
	}
	else if( strncmp( profile, "ps_", 3 ) == 0 )
	{
		orbisProfile = "sce_ps_orbis";
	}
	else
	{
		return false;
	}
	std::string commandLine = "orbis-psslc -profile " + orbisProfile + " -entry " + entryPoint + " -o " + outFile + " " + inFile;
	if( !RunProcess( commandLine.c_str() ) )
	{
		g_messages.AddMessage( "\\memory(0): error O0100: external Orbis compiler returned error1" );
		return false;
	}

	FILE* output;
	if( fopen_s( &output, outFile, "rb" ) != 0 )
	{
		g_messages.AddMessage( "\\memory(0): error O0100: external Orbis compiler returned error2" );
		return false;
	}
	...
	if( length == 0 )
	{
		g_messages.AddMessage( "\\memory(0): error O0100: external Orbis compiler returned error3" );
		return false;
	}
	D3DCreateBlob( length, orbisEffectData );
	fseek( output, 0, SEEK_SET );
	fread( ( *orbisEffectData )->GetBufferPointer(), 1, length, output );
	fclose( output );
...
void ConvertOrbisSemantics( InputStageType stageType, ASTNode* shaderNode, std::map<Symbol*, InlineString>& bkSemantics )
{
	Symbol* entryPointSymbol = shaderNode->GetChild( 1 )->GetSymbol();
	ASTNode* functionHeader  = entryPointSymbol->definition->GetChildOrNull( 0 );

	if( stageType == VERTEX_STAGE )
	{

		const char* position[] = { "position", "sv_position", nullptr };
		std::vector<Symbol*> positionPath;
		if( FindOutputBySemantics( functionHeader, position, &positionPath ) )
		{
			static const char* s_position = "S_POSITION";
			if( positionPath.back() )
			{
				bkSemantics[positionPath.back()] = positionPath.back()->semantic;
				positionPath.back()->semantic = MakeInlineString( s_position );
			}
			else
			{
				bkSemantics[entryPointSymbol] = entryPointSymbol->semantic;
				entryPointSymbol->semantic = MakeInlineString( s_position );
			}
		}
	}
	else if( stageType == PIXEL_STAGE )
	{
		Symbol* entryPointSymbol = shaderNode->GetChild( 1 )->GetSymbol();
		ASTNode* functionHeader  = entryPointSymbol->definition->GetChildOrNull( 0 );
		...
		const char* position[] = { "position", "sv_position", nullptr };
		std::vector<Symbol*> positionPath;
		if( FindParameterBySemantics( functionHeader, position, &positionPath ) )
		{
			static const char* s_position = "S_POSITION";
			bkSemantics[positionPath.back()] = positionPath.back()->semantic;
			positionPath.back()->semantic = MakeInlineString( s_position );
		...
		positionPath.clear();
		if( FindOutputBySemantics( functionHeader, target, &positionPath ) )
		{
			static const char* s_position = "S_TARGET_OUTPUT";
			if( positionPath.back() )
			{
				bkSemantics[positionPath.back()] = positionPath.back()->semantic;
				positionPath.back()->semantic = MakeInlineString( s_position );
		...
		const char* depth[] = { "depth", nullptr };
		positionPath.clear();
		if( FindOutputBySemantics( functionHeader, depth, &positionPath ) )
		{
			static const char* s_position = "S_DEPTH_OUTPUT";
			if( positionPath.back() )
			{
				bkSemantics[positionPath.back()] = positionPath.back()->semantic;
				positionPath.back()->semantic = MakeInlineString( s_position );
...
bool EffectCompilerOrbis::Create()
{
	GetOperatorSymbol( 0 );
	return true;
}

bool EffectCompilerOrbis::CompileEffect( const char* source, size_t sourceLength, const D3DXMACRO* defines, ID3DXInclude* include, EffectData& result )
{
	...
	for( size_t passIx = 0; passIx < techniqueNode->GetChildrenCount(); ++passIx )
	{
		...
		for( size_t stateIx = 0; stateIx < passNode->GetChildrenCount(); ++stateIx )
		{
			...
			std::map<Symbol*, InlineString> bkSemantics;
			ConvertOrbisSemantics( stage.type, shaderNode, bkSemantics );

			CComPtr<ID3DBlob> orbisEffectData;
			{
				CompilerInputStream os( state );
				PrintHLSL11( os, state.GetTree(), 0 );
				...
				if( !CompileOrbisShader( code.c_str(), patchEntryPoint.c_str(), profile.c_str(), &orbisEffectData ) )
				{
					return false;
				}
			}
			for( auto it = bkSemantics.begin(); it != bkSemantics.end(); ++it )
			{
				it->first->semantic = it->second;
			}
