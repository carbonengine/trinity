		...
		for( auto& pass : technique.passes )
		{
			for( auto& stage : pass.stageInputs )
			{
				for( auto& constant : stage.constants )
				{
					...
					{
						auto found = std::find_if( annotations->begin(), annotations->end(), []( const Tr2EffectParameterAnnotation& annotation ) { return strcmp( annotation.name, "IsBlendable" ) == 0; } );
						if( found == annotations->end() || found->type != Tr2EffectParameterAnnotation::BOOL || !found->boolValue )
						{
							continue;
						}
					}

					auto AddBlendableParameter = [&]( auto& blendableParameters ) {
						...
						memcpy( &defaultValue, stage.constantValues + constant.offset, sizeof( ValueType ) );
					...
					{
					case 1:
						AddBlendableParameter( blendableFloatParameters );
						break;
					case 2:
						AddBlendableParameter( blendableVector2Parameters );
						break;
					case 3:
						AddBlendableParameter( blendableVector3Parameters );
						break;
					case 4:
						AddBlendableParameter( blendableVector4Parameters );
						break;
					default:
						break;
					}
				...
				{
					m_requiresSourceTexture = find_if( stage.resources.begin(), stage.resources.end(), []( const auto& texture ) { return strcmp( texture.second.name, "Blit" ) == 0; } ) != stage.resources.end();
				}
			}
		}
	}
...
void Tr2AccumulatedGenericEffects::GenericEffectInstance::SetParameters()
{
	for( auto& param : effect->blendableFloatParameters )
	{
		...
		std::swap( floatValues[&param - &effect->blendableFloatParameters[0]], value );
		param.parameter->SetValue( value );
	}
	for( auto& param : effect->blendableVector2Parameters )
	{
		auto value = param.parameter->GetValue();
		std::swap( vector2Values[&param - &effect->blendableVector2Parameters[0]], value );
		param.parameter->SetValue( value );
	}
	for( auto& param : effect->blendableVector3Parameters )
	{
		auto value = param.parameter->GetValue();
		std::swap( vector3Values[&param - &effect->blendableVector3Parameters[0]], value );
		param.parameter->SetValue( value );
	}
	for( auto& param : effect->blendableVector4Parameters )
	{
		auto value = param.parameter->GetValue();
		std::swap( vector4Values[&param - &effect->blendableVector4Parameters[0]], value );
		param.parameter->SetValue( value );
	}
}
...
void AccumulateGenericEffects( Tr2PostProcess2& postprocess, std::vector<Tr2PostProcessAttributes*>& sources )
{
	...
	for( auto& bucket : buckets )
	{
		...
		postprocess.m_genericEffects.effects[bucket.effects.front().effect->m_executionSlot].push_back( instance );
	}
