...
{
#define HANDLE_PLATFORM_CAP(x) case x: return TRINITY_PLATFORM_SUPPORTS_##x != 0;
#define HANDLE_APPLICATION_CAP(x) case x: return TRINITY_SUPPORTS_##x != 0;

	switch( cap )
	{
	HANDLE_PLATFORM_CAP( NON_SYNCHRONIZED_LOCKS );
