...
{
	Symbol* payloadType = nullptr;

	for( auto childNode : libNode->GetChildren() )
	{
		if( childNode->GetNodeType() == NT_STATE_ASSIGNMENT && EqualsCaseInsensitive( childNode->GetToken()->stringValue, "payloadtype" ) )
		{
			if( payloadType )
			{
				state.ShowMessage( childNode->GetToken()->fileLocation, EC_CUSTOM_ERROR, "duplicate PayloadType state" );
				return nullptr;
			}
			else
			{
				...
				payloadType = state.GetSymbolTable().LookupGlobal( value[0].stringValue.c_str() );
				if( !payloadType )
				{
					state.ShowMessage( childNode->GetToken()->fileLocation, EC_CUSTOM_ERROR, "undefined PayloadType \"%s\"", value[0].stringValue.c_str() );
					return nullptr;
				}
				if( !payloadType->isTypeName )
				{
					state.ShowMessage( childNode->GetToken()->fileLocation, EC_CUSTOM_ERROR, "PayloadType \"%s\" is not a type", value[0].stringValue.c_str() );
					return nullptr;
				}
			}
		}
	...
	if( !payloadType )
	{
		state.ShowMessage( libNode->GetLocation(), EC_CUSTOM_ERROR, "library must define PayloadType state" );
		return nullptr;
	}
	return payloadType;
}