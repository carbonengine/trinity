...
{
	Tr2RenderTargetAL rt;
	ASSERT_HRESULT_FAILED( rt.Create( 128, 128, 1, PIXEL_FORMAT_B8G8R8A8_UNORM, 1, 0, *renderContext ) );
	EXPECT_FALSE( rt.IsValid() );
	EXPECT_FALSE( rt.GetTexture().IsValid() );
}
...
TEST_F( WithValidRenderContext, RenderTargetIsValidAfterCreation )
{
	...
	EXPECT_EQ( 1, rt.GetMsaaType() );
	EXPECT_EQ( 0, rt.GetMsaaQuality() );
...
{
	Tr2RenderTargetAL rt;
	ASSERT_HRESULT_SUCCEEDED( rt.Create( 128, 64, 0, PIXEL_FORMAT_B8G8R8A8_UNORM, *renderContext ) );
	EXPECT_TRUE( rt.IsValid() );
	EXPECT_TRUE( rt.IsReadable() );
	EXPECT_TRUE( rt.GetTexture().IsValid() );
	EXPECT_EQ( 128, rt.GetWidth() );
...
{
	Tr2RenderTargetAL rt;
	ASSERT_HRESULT_SUCCEEDED( rt.Create( 128, 64, 1, PIXEL_FORMAT_B8G8R8A8_UNORM, 4, 0, *renderContext ) );
	EXPECT_TRUE( rt.IsValid() );
	EXPECT_FALSE( rt.IsReadable() );
	EXPECT_EQ( 128, rt.GetWidth() );
	EXPECT_EQ( 64, rt.GetHeight() );
	EXPECT_EQ( 1, rt.GetMipCount() );
	EXPECT_EQ( PIXEL_FORMAT_B8G8R8A8_UNORM, rt.GetFormat() );
	EXPECT_EQ( 4, rt.GetMsaaType() );
	EXPECT_EQ( 0, rt.GetMsaaQuality() );
}
...
{
	Tr2RenderTargetAL rt;
	ASSERT_HRESULT_SUCCEEDED( rt.Create( 128, 128, 1, PIXEL_FORMAT_B8G8R8A8_UNORM, 1, 0, *renderContext ) );
	rt.Destroy();
	EXPECT_FALSE( rt.IsValid() );
}
...
TEST_F( WithValidRenderContext, CanResolveMsaaRenderTarget )
{
	...
	ASSERT_HRESULT_SUCCEEDED( rtMsaa.Create( 128, 64, 1, PIXEL_FORMAT_B8G8R8A8_UNORM, 4, 0, *renderContext ) );

	Tr2RenderTargetAL rt;
	ASSERT_HRESULT_SUCCEEDED( rt.Create( 128, 64, 0, PIXEL_FORMAT_B8G8R8A8_UNORM, 1, 0, *renderContext ) );
...
TEST_F( WithValidRenderContext, DifferentRenderTargetsAreNotEqual )
{
	...
	ASSERT_HRESULT_SUCCEEDED( rt1.Create( 128, 64, 0, PIXEL_FORMAT_B8G8R8A8_UNORM, 1, 0, *renderContext ) );
	Tr2RenderTargetAL rt2;
	ASSERT_HRESULT_SUCCEEDED( rt2.Create( 128, 64, 0, PIXEL_FORMAT_B8G8R8A8_UNORM, 1, 0, *renderContext ) );
...
TEST_F( WithValidRenderContext, CanGetLockedRenderTarget )
{
	Tr2RenderTargetAL rt;
	...
	ASSERT_TRUE( lockedRT.IsValid() );

	void* data = nullptr;
	uint32_t pitch = 0;
	ASSERT_HRESULT_SUCCEEDED( lockedRT.Lock( data, pitch, *renderContext ) );
	EXPECT_NE( nullptr, data );
	EXPECT_LE( 4 * 128u, pitch );
