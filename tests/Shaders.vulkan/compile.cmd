...
for %%i in (*.psh) do (
    ...
    %FXC% -S frag -V -D -e main --sub 32 --ssb 32 --suavb 224 --stb 416 -DPATCHED_SHADER=1 -o %%~ni.ps.patched.bin %%i
)
for %%i in (*.vsh) do (
    %FXC% -S vert -V -D -e main --sub 0 --ssb 0 --suavb 192 --stb 384 -o %%~ni.vs.bin %%i
    %FXC% -S vert -V -D -e main --sub 0 --ssb 0 --suavb 192 --stb 384 -DPATCHED_SHADER=1 -o %%~ni.vs.patched.bin %%i
)
for %%i in (*.csh) do (
